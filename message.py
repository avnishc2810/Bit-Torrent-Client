...
'''
    def dispatch(self):
        ...
        # Log the seeding activity if a Piece message is sent
        if message_id == 7:
            logging.info("Seeding: A piece message is being sent.")
...
class UnChoke(Message):
    ...
    def to_bytes(self):
        logging.info("Sending 'UnChoke' message. Allowing peer to download.")
        return pack(">IB", self.payload_length, self.message_id)
...
class Interested(Message):
    ...
    def to_bytes(self):
        logging.info("Received 'Interested' message. Peer wants to download.")
        return pack(">IB", self.payload_length, self.message_id)
...
class Piece(Message):
    ...
    def to_bytes(self):
        Piece.upload_count += 1
        logging.info(f"Seeding: Sent piece {self.piece_index} at offset {self.block_offset}. "f"Total pieces uploaded: {Piece.upload_count}")
